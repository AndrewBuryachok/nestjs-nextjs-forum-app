...
import { createLandmarkAction, createLandmarkWithUserAction } from '../actions';
import { createLandmarkSchema, CreateLandmarkType } from '../schema';
...
export default function CreateLandmarkForm(props: Props) {
  ...
  const form = useForm<CreateLandmarkType>({
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    const res = props.isAll
      ? await createLandmarkWithUserAction({ ...data, userId })
      : await createLandmarkAction(data);
    if (res.data) {
      if (res.data.ok) {
        const title = t('toasts.landmarks.create.success');
        toaster.success({ title });
        router.refresh();
        closeDialog();
      } else {
        const title = res.data.message ?? t('toasts.landmarks.create.failure');
        toaster.error({ title });
      }
    }
  });
