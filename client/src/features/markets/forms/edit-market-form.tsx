...
import { editMyMarketAction, editUserMarketAction } from '../actions';
import { editMarketSchema, EditMarketType } from '../schema';
...
export default function EditMarketForm(props: Props) {
  ...
  const form = useForm<EditMarketType>({
  ...
  const onSubmit = form.handleSubmit(async (data) => {
    const res = props.isAll
      ? await editUserMarketAction(data)
      : await editMyMarketAction(data);
    if (res.data) {
      if (res.data.ok) {
        const title = t('toasts.markets.edit.success');
        toaster.success({ title });
        router.refresh();
        closeDialog();
      } else {
        const title = res.data.message ?? t('toasts.markets.edit.failure');
        toaster.error({ title });
      }
    }
  });
